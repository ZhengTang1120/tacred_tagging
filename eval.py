...
for i, p in enumerate(predictions):
    ...
    attns = [[attns[i][k][j] for j in range(len(inputs[i])) if inputs[i][j] != '[PAD]'] for k in range(16)]
    output[-1]['predicted_tags'] = [[1 if attns[k][j] in sorted(attns[k])[:5] else 0 for j in range(attns[k])] for k in range(16)]
