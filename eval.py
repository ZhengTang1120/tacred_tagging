...
for i, p in enumerate(predictions):
    ...
    chunked_attn = [[softmax(attns[i][k][:len(output[-1]['raw_words'])], axis=0).tolist()] for k in range(16)]
    output[-1]['predicted_tags'] = [[chunked_attn[k][j] for j in range(len(chunked_attn[k]))] for k in range(16)]
    print (output[-1]['predicted_tags'][0])
